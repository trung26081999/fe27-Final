...
const ProductDetail = () => {
  ...
  const addToCart = () => {
    if (!user) {
      ...
      const checkProductUser = cartState.data.some(
        (item) => item.userEmail === user.email,
      )
      const productFindId = checkProduct
        ? cartState.data.find((item) => item?.productId === id)
        : null
      ...
      if (
        ...
        checkProductUser &&
        productFindId &&
        productDetail?.size === productFindId?.size
      ) {
        dispatch(
          updateProductCartAction({
            id: productFindId.id,
            data: {
              quantity: productFindId.quantity + productDetail?.quantity,
            },
          }),
        )
        dispatch(getListCartAction())
        ...
        <Wrapper>
          <ImgContainer>
            {loading ? (
              ...
              <Image width={350} src={productDetail?.img} />
            )}
          </ImgContainer>
          <InfoContainer>
